...
var buttonType = z.enum(["forward", "backward"]);
var button = z.object({
  type: buttonType,
...
// src/tools/isTouchDevice.ts
var isTouchDevice = () => {
  return "ontouchstart" in window || navigator.maxTouchPoints > 0;
};
var isTouchDevice_default = isTouchDevice;

// src/types/config/fixed.ts
var fit = z.enum(["width", "height", "page"]);
var fixedOptions = z.object({
  fit: fit.optional().default("page")
});
var fixedConfig = z.object({
  fit: fit.optional()
});

// src/types/config/flow.ts
var fontSize = z.number().min(8).max(24);
var maxColumns = z.number().min(1).max(4);
var minCharsPerColumn = z.number().min(50).max(70);
var maxCharsPerColumn = z.number().min(70).max(90);
var flowOptions = z.object({
...
var layout = z.enum(["fixed", "flow"]);
var direction = z.enum(["horizontal", "vertical"]);
var touch = z.boolean();
var buttons = z.array(button).optional();
...
var commonOptions = z.object({
  direction: direction.optional(),
  touch: touch.optional(),
  buttons
});
var options = z.discriminatedUnion("layout", [
  z.object({
    ...
    for (let i = 0; i < buttons2.length; i++) {
      ...
      if (option) {
        ...
        btn.addEventListener("pointerdown", (ev) => {
          if (ev.button === 0) {
            reader.scrollLeft += reader.clientWidth;
          }
        });
...
// src/utils/flow/setup.ts
var charWidthFactor = 1.65;
var getColumnNumber = (config3) => {
  const {
    fontSize: fontSize2,
    maxColumns: absoluteMaxColumns,
    minCharsPerColumn: minCharsPerColumn2,
    maxCharsPerColumn: maxCharsPerColumn2
  } = config3;
  ...
  const reader = document.getElementById("reader");
  if (!reader) return;
  const { clientWidth } = reader;
  const charWidth = fontSize2 / charWidthFactor;
  const minColumnWidth = minCharsPerColumn2 * charWidth;
  const maxColumnWidth = maxCharsPerColumn2 * charWidth;
  const minColumns = Math.floor(clientWidth / maxColumnWidth);
  const maxColumns2 = Math.floor(clientWidth / minColumnWidth);
  const columnNumber = Math.max(
    1,
    minColumns,
    Math.min(maxColumns2, absoluteMaxColumns)
  );
  console.log({
    clientWidth,
    minColumnWidth,
...
var setup = (flowOptions2) => {
  const { fontSize: fontSize2, maxColumns: maxColumns2, minCharsPerColumn: minCharsPerColumn2, maxCharsPerColumn: maxCharsPerColumn2 } = flowOptions2;
  const config3 = {
    fontSize: fontSize2,
    maxColumns: maxColumns2,
    minCharsPerColumn: minCharsPerColumn2,
...
var setupDomEvents = () => {
  ...
  const checkIfScreenXBorderIsPressed = (e) => {
    const touchX = e.x;
    if (touchX) {
      const threshold = 20;
  ...
  const handleLongPress = () => {
    ...
    alert("Has hecho una pulsaci\xF3n larga");
  };
...
var setupDomEvents_default = setupDomEvents;

// src/utils/setup.ts
var setup2 = (initialOptions) => {
...
${JSON.stringify(res.error.issues)}`
  ...
  setupDomEvents_default();
  const commonConfig2 = {
    buttons: res.data.options.buttons,
    touch: res.data.options.touch ?? isTouchDevice_default()
  };
  if (commonConfig2.buttons?.length) {
    render_default(commonConfig2.buttons);
  }
  if (res.data.layout === "flow") {
    const flowConfig2 = setup_default(res.data.options);
    return {
      layout: "flow",
      ...commonConfig2,
      ...flowConfig2
...
window.onload = () => {
  ...
  setConfig(config3);
  document.body.classList.add(config3.layout);
